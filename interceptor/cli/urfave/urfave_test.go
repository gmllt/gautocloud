...
type SchemaFake struct {
	...
	FooBar  bool
	Found   string
	Default string `cloud-default:"def-val"`
}
...
var _ = Describe("Urfave", func() {
	...
	Context("schema is not a pointer", func() {
		...
		It("should put flags into schema provided by user and not overriding existing value", func() {
			ran := false
			i := NewCli()
			action := func(c *cli.Context) error {
				i.SetContext(c)
				...
				Expect(err).ToNot(HaveOccurred())
				schema := final.(SchemaFake)
				Expect(schema.Default).Should(Equal("bar"))
				return nil
			}
