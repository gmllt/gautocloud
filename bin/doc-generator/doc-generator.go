...
func getDocMap() map[string]Doc {
	...
	for id, conn := range gautocloud.Connectors() {
		...
		if _, ok := docs[rootID]; !ok {
			docs[rootID] = Doc{
				Name:        cases.Title(language.Und).String(rootID),
		...
		connDoc := DocConnector{
			Name:        cases.Title(language.Und).String(rootID + " - " + connName),
			ID:          conn.Id(),
			Pkg:         pkgConn,
			...
			DocURL:      docURL,
			Tip:         tip,
			GlobalType:  cases.Title(language.Und).String(globalType),
			SimpleName:  cases.Title(language.Und).String(connName),
			StructGiven: generateDocStruct(givenData),
			Closeable:   isCloseable(givenData),
		}
		doc := docs[rootID]
		doc.Connectors = append(doc.Connectors, connDoc)
		sort.Sort(doc.Connectors)
		docs[rootID] = doc
	}
	return docs
}
